...
ok('known bugs tile present', text.includes('Known bugs'))
ok('test case version chip present', /Test cases v\d/.test(text))
await shot('f4_dashboard_manager')
await page.evaluate(() => {
  const btn = [...document.querySelectorAll('button')].find(b => b.textContent.trim() === '24 hours')
  btn?.click()
})
await page.waitForFunction(() => document.body.innerText.includes('Hourly pass rate'), { timeout: 6000 }).catch(() => {})
ok('24-hour hourly view renders', (await bodyText()).includes('Hourly pass rate · last 24 hours'))
await shot('f4b_dashboard_24h')
