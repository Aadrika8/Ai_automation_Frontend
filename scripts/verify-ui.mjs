...
ok('manager has no refresh button', !text.includes('Refresh from Excel'))
...
await page.waitForFunction(
  () => /Camera testing|No data loaded yet/.test(document.body.innerText), { timeout: 6000 })
text = await bodyText()
ok('unit data view renders sections or empty state',
  text.includes('Camera testing') || text.includes('No data loaded yet'))
...
await page.waitForFunction(() => document.body.innerText.includes('Unit Testing'), { timeout: 6000 })
text = await bodyText()
ok('qa sees refresh buttons', text.includes('Refresh from Excel'))
...
await clickButton('Refresh from Excel')
ok('sync dialog opens',
  /Reading from|Excel root folder|does not exist|not configured/.test(await bodyText()))
await shot('f6_sync_dialog')
await page.keyboard.press('Escape')
await clickButton('Upload Excel')
ok('upload dialog opens', (await bodyText()).includes('Drop an Excel file here'))
await shot('f6b_upload_dialog')
await page.keyboard.press('Escape')
await goSettled(`${BASE}/settings`)
